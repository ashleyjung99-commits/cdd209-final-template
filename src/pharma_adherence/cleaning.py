...
def clean_prescription_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["pharmacy_name"] = df["pharmacy_name"].replace(r"#\d+$", "", regex=True).str.strip()
    ...
    # Drop rows if missing critical fields: "patient_id", "fill_date", "drug_name"
    critical_cols = ["patient_id", "fill_date", "drug_name"]
    df = df.dropna(subset=critical_cols)

    # Sort by "patient_id", "fill_date"
